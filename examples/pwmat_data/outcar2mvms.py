...
def run_convert_configs(cmd_list:list[str]):
    ...
    parser.add_argument('-s', '--savepath',      type=str, required=False, default="./", help="The output dir path, if not provided, the current dir will be used")
    args = parser.parse_args(cmd_list)
    image = Config(data_path=args.input, format="vasp/outcar")
    image.to(format="pwmat/movement", data_path="./")
    print("covert outcar to movement done!")

if __name__=="__main__":
    # os.chdir("/data/home/wuxingxing/codespace/pwdata_dev/examples/vasp_data")
    print(sys.argv)
